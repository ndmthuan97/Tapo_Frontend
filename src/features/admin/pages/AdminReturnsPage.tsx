...
const STATUS_CONFIG: Record<ReturnRequestStatus, { color: string; bg: string; border: string; icon: React.ElementType }> = {
  PENDING:  { color: 'text-amber-700 dark:text-amber-300',    bg: 'bg-amber-50 dark:bg-amber-500/10',    border: 'border-amber-200 dark:border-amber-500/20',    icon: Clock       },
  APPROVED: { color: 'text-emerald-700 dark:text-emerald-300', bg: 'bg-emerald-50 dark:bg-emerald-500/10', border: 'border-emerald-200 dark:border-emerald-500/20', icon: CheckCircle2 },
  REJECTED: { color: 'text-red-600 dark:text-red-400',         bg: 'bg-red-50 dark:bg-red-500/10',         border: 'border-red-200 dark:border-red-500/20',         icon: XCircle     },
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4"
      ...
      <div className="w-full max-w-md rounded-2xl bg-white dark:bg-[#21232d] shadow-2xl p-6">
        ...
        <textarea value={note} onChange={e => setNote(e.target.value)}
          placeholder={action === 'approve' ? t('adminReturns.noteModal.approvePh') : t('adminReturns.noteModal.rejectPh')}
          rows={3}
        ...
        <div className="flex justify-end gap-2 mt-4">
          <button onClick={onClose} className="rounded-xl border border-gray-200 dark:border-white/10 px-4 py-2 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">{t('adminReturns.noteModal.cancel')}</button>
          <button onClick={() => {
            if (action === 'reject' && !note.trim()) { toast.error(t('adminReturns.noteModal.rejectRequired')); return }
            onConfirm(note.trim())
            ...
            {action === 'approve' ? t('adminReturns.noteModal.confirmApprove') : t('adminReturns.noteModal.confirmReject')}
          </button>
        </div>
      </div>
    </div>
      ...
      <div className="w-full max-w-lg rounded-2xl bg-white dark:bg-[#21232d] shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-white/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold text-gray-900 dark:text-white">{t('adminReturns.detail.title')} #{request.orderCode}</h2>
              <span className={cn('inline-flex items-center gap-1 text-[10px] font-semibold', cfg.color)}>
                <Icon size={10} /> {t(`adminReturns.status.${request.status}`)}
              </span>
            </div>
          </div>
        ...
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <p className="text-[10px] text-gray-400 mb-1">{t('adminReturns.detail.customer')}</p>
              <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">{request.userName}</p>
            </div>
            <div>
              <p className="text-[10px] text-gray-400 mb-1">{t('adminReturns.detail.date')}</p>
              <p className="text-sm text-gray-700 dark:text-gray-300">{new Date(String(request.createdAt)).toLocaleDateString('vi-VN')}</p>
            </div>
          </div>
          <div>
            <p className="text-[10px] text-gray-400 mb-1">{t('adminReturns.detail.reason')}</p>
          ...
          {request.evidenceImages && request.evidenceImages.length > 0 && (
            <div>
              <p className="text-[10px] text-gray-400 mb-2">{t('adminReturns.detail.evidence', { count: request.evidenceImages.length })}</p>
          ...
          {request.staffNote && (
            <div>
              <p className="text-[10px] text-gray-400 mb-1">{t('adminReturns.detail.note')}</p>
...
function AdminReturnsPage() {
  ...
  const STATUS_FILTER_OPTIONS = useMemo(() => [
    { value: 'ALL',      label: t('adminReturns.filter.ALL')      },
    { value: 'PENDING',  label: t('adminReturns.filter.PENDING')  },
    { value: 'APPROVED', label: t('adminReturns.filter.APPROVED') },
    { value: 'REJECTED', label: t('adminReturns.filter.REJECTED') },
  ...
  const loadStats = useCallback(async () => {
    try {
      const [all, pending, approved] = await Promise.all([
        returnRequestApi.adminListAll({ page: 0, size: 1 }),
        returnRequestApi.adminListAll({ page: 0, size: 1, status: 'PENDING'  }),
        returnRequestApi.adminListAll({ page: 0, size: 1, status: 'APPROVED' }),
      ])
      if (all.data)      setStatTotal(all.data.totalElements)
      if (pending.data)  setStatPending(pending.data.totalElements)
      if (approved.data) setStatApproved(approved.data.totalElements)
    } catch { /* ignore */ }
  }, [])
  ...
  function loadDataAndStats() { loadData(); loadStats() }
  ...
  async function handleAction(id: string, status: ReturnRequestStatus, note: string) {
    ...
    if (res.success) {
      toast.success(status === 'APPROVED' ? t('adminReturns.toast.approved') : t('adminReturns.toast.rejected'))
      loadDataAndStats()
    } else {
      toast.error(t('adminReturns.toast.error'), { description: res.error?.message })
    }
  }
  ...
  return (
    <>
      <div className="p-6 space-y-6">
        {/* Title */}
        <div>
          <h1 className="text-xl font-bold text-gray-900 dark:text-white tracking-tight">{t('adminReturns.title')}</h1>
        </div>

        {/* Stat cards */}
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <StatCard icon={RotateCcw}    label={t('adminReturns.statTotal')}    value={statTotal}    color="bg-rose-500"    />
          <StatCard icon={Clock}        label={t('adminReturns.statPending')}  value={statPending}  color="bg-amber-500"   />
          <StatCard icon={CheckCircle2} label={t('adminReturns.statApproved')} value={statApproved} color="bg-emerald-500" />
        </div>
        ...
        <div className="rounded-2xl border border-gray-100 dark:border-white/5 bg-white dark:bg-[#21232d] shadow-sm overflow-hidden transition-colors">
          ...
          <div className="flex flex-wrap items-center gap-3 border-b border-gray-100 dark:border-white/5 px-5 py-4">
            <p className="text-sm font-semibold text-gray-700 dark:text-gray-200 mr-auto">{t('adminReturns.tableTitle')}</p>
            <AdminSearchInput value={searchQuery} onChange={v => { setSearchQuery(v); setPage(0) }} placeholder={t('adminReturns.searchPh')} />
            ...
            <button
              onClick={() => { exportReturnsToExcel(filteredRequests); toast.success(t('adminReturns.exportSuccess')) }}
            ...
            >
              <Download size={12} /> {t('adminReturns.exportExcel')}
            </button>
            <button onClick={loadData} className="flex items-center gap-1.5 rounded-xl border border-gray-200 dark:border-white/10 px-3 py-2 text-xs font-medium text-gray-600 dark:text-gray-400 hover:border-orange-400 hover:text-orange-500 transition-colors">
              <RefreshCw size={12} /> {t('adminReturns.refresh')}
            </button>
          </div>
            ...
            <table className="w-full min-w-[640px] text-sm">
              <thead>
                <tr className="text-left text-[11px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 border-b border-gray-100 dark:border-white/5">
                  <th className="px-5 py-3.5">{t('adminReturns.col.orderCode')}</th>
                  <th className="px-5 py-3.5">{t('adminReturns.col.customer')}</th>
                  <th className="px-5 py-3.5">{t('adminReturns.col.reason')}</th>
                  <th className="px-5 py-3.5">{t('adminReturns.col.date')}</th>
                  <th className="px-5 py-3.5">{t('adminReturns.col.status')}</th>
                  <th className="px-5 py-3.5 text-right">{t('adminReturns.col.actions')}</th>
                </tr>
              </thead>
                ...
                {isLoading ? (<ReturnSkeleton />) : filteredRequests.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="py-16 text-center">
                      ...
                      <p className="text-sm text-gray-400">{t('adminReturns.empty')}</p>
                    </td>
                  </tr>
                    ...
                    <tr key={req.id} className="group transition-colors hover:bg-orange-50/60 dark:hover:bg-white/[0.03]">
                      ...
                      <td className="px-5 py-3.5">
                        <span className={cn('inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-semibold border', cfg.color, cfg.bg, cfg.border)}>
                          <SIcon size={10} /> {t(`adminReturns.status.${req.status}`)}
                        </span>
                      </td>
      ...
      {noteModal && (
        <NoteModal
          title={noteModal.action === 'approve' ? t('adminReturns.noteModal.approveTitle') : t('adminReturns.noteModal.rejectTitle')}
