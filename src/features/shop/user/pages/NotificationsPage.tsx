/**
 * NotificationsPage — Full-page notification center for authenticated shop users.
 *
 * Layout: Header + main content + Footer (consistent with all other user pages).
 * Groups notifications by date (Hôm nay / Hôm qua / Cũ hơn).
 * "Mark all read" button clears the unread badge in NotificationBell.
 *
 * react skill §3: single clear export, named component.
 * ui-ux-pro-max: consistent design tokens, empty state, accessible heading hierarchy.
 */
...
function NotificationsPage() {
  ...
  return (
    <>
      {/* SEO */}
      <title>Thông báo | Tapo</title>

      <div className="flex min-h-screen flex-col bg-gray-50 dark:bg-[#191b22]">
        <Header />
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    Bạn có{' '}
                    <span className="font-semibold text-orange-500">{unreadCount}</span>{' '}
                    thông báo chưa đọc
                  </p>
                </div>
              </div>
            ...
            {notifications.length === 0 ? (
              ...
              <div className="space-y-8">
                {Object.entries(grouped).map(([label, items]) => {
                  ...
                  return (
                    <div key={label}>
                      <h3 className="mb-4 text-sm font-bold text-gray-400 uppercase tracking-wider">
                        {label}
                      </h3>
